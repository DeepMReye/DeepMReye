#!/usr/bin/env python3
"""Targeted Exploration to Beat Supervised fold-pca:64 on 1-TR Mean.

Evaluates:
1. corpus-pca:64 (Unsupervised PCA on 1800 subjects vs Supervised fold-pca on labeled subjects)
2. corpus-pca:128
3. lr-cca:32 + corpus-pca:32 (Hybrid 64 features)
4. lr-cca:32 + corpus-pca:64 (Hybrid 96 features)
5. lr-cca:32 + diff-pca:32 (Hybrid 64 features)
6. lr-cca:32 + band-pca:32 (Hybrid 64 features)
7. lr-cca:32 + corpus-pca:32 + diff-pca:16 + gev-fast:16 (Multi-domain 96 features)
8. Multi-Head Weighted Ensembling / Adaptive Shrinkage
"""
import sys
import time
...
def main():
    ...
    experiments.append(("1. corpus-pca:64 (Unsupervised PCA on N=1800)", feat_cpca64))

    # 2. corpus-pca:128
    def feat_cpca128(s):
        return (s["vox"] - pca_mu) @ pca_comp[:, :128]
    experiments.append(("2. corpus-pca:128 (Unsupervised PCA on N=1800)", feat_cpca128))

    # 3. lr-cca:32 + corpus-pca:32
    def feat_cca32_pca32(s):
        xc = s["vox"] - mu_cca
        z_cca = 0.5 * (xc[:, li] @ wl[:, :32] + xc[:, ri] @ wr[:, :32])
        z_pca = (s["vox"] - pca_mu) @ pca_comp[:, :32]
        return np.concatenate([z_cca, z_pca], axis=1)
    experiments.append(("3. DeepMReye 2.0 (lr-cca:32 + corpus-pca:32 = 64 features)", feat_cca32_pca32))

    # 4. lr-cca:32 + corpus-pca:64
    def feat_cca32_pca64(s):
        xc = s["vox"] - mu_cca
        z_cca = 0.5 * (xc[:, li] @ wl[:, :32] + xc[:, ri] @ wr[:, :32])
        z_pca = (s["vox"] - pca_mu) @ pca_comp[:, :64]
        return np.concatenate([z_cca, z_pca], axis=1)
    experiments.append(("4. DeepMReye 2.0 (lr-cca:32 + corpus-pca:64 = 96 features)", feat_cca32_pca64))

    # 5. lr-cca:32 + diff-pca:32
    def feat_cca32_diff32(s):
        xc = s["vox"] - mu_cca
        z_cca = 0.5 * (xc[:, li] @ wl[:, :32] + xc[:, ri] @ wr[:, :32])
        z_diff = (s["vox"] - diff_mu) @ diff_comp[:, :32]
        return np.concatenate([z_cca, z_diff], axis=1)
    experiments.append(("5. DeepMReye 2.0 (lr-cca:32 + diff-pca:32 = 64 features)", feat_cca32_diff32))

    # 6. lr-cca:32 + band-pca:32
    def feat_cca32_band32(s):
        xc = s["vox"] - mu_cca
        z_cca = 0.5 * (xc[:, li] @ wl[:, :32] + xc[:, ri] @ wr[:, :32])
        z_band = (s["vox"] - band_mu) @ band_comp[:, :32]
        return np.concatenate([z_cca, z_band], axis=1)
    experiments.append(("6. DeepMReye 2.0 (lr-cca:32 + band-pca:32 = 64 features)", feat_cca32_band32))

    # 7. lr-cca:32 + corpus-pca:32 + diff-pca:16 + gev-fast:16
    def feat_multi96(s):
        xc = s["vox"] - mu_cca
        z_cca = 0.5 * (xc[:, li] @ wl[:, :32] + xc[:, ri] @ wr[:, :32])
        z_pca = (s["vox"] - pca_mu) @ pca_comp[:, :32]
        ...
        return np.concatenate([z_cca, z_pca, z_diff, z_gev], axis=1)
    experiments.append(("7. DeepMReye 2.0 Super-Basis (cca:32 + pca:32 + diff:16 + gev:16 = 96)", feat_multi96))

    # 8. lr-cca:32 + corpus-pca:64 + gev-fast:16 (112 features)
    def feat_super112(s):
        xc = s["vox"] - mu_cca
        z_cca = 0.5 * (xc[:, li] @ wl[:, :32] + xc[:, ri] @ wr[:, :32])
        z_pca = (s["vox"] - pca_mu) @ pca_comp[:, :64]
        z_gev = (s["vox"] - gev_mu) @ gev_comp[:, :16]
        return np.concatenate([z_cca, z_pca, z_gev], axis=1)
    experiments.append(("8. DeepMReye 2.0 Super-Basis (cca:32 + pca:64 + gev:16 = 112 features)", feat_super112))

    # Baseline reference
    print("\n" + "=" * 105)
    print(f"{'Configuration':<68} {'1-TR Mean LODO r':>18} {'Delta vs Baseline':>14}")
    print("=" * 105)
    print(f"{'fold-pca:64 (Supervised Reference Baseline)':<68} {0.8370:>18.4f} {'(Baseline)':>14}")
    print("-" * 105)

    for name, fn in experiments:
        t0 = time.time()
        r_val, per_fold = eval_lodo_subtr(subs, fn)
        delta = r_val - 0.8370
        sign = "+" if delta >= 0 else ""
        winner = " 🏆" if delta > 0 else ""
        print(f"{name:<68} {r_val:>18.4f} {f'({sign}{delta:.4f})':>14}{winner}  [{time.time() - t0:.1f}s]", flush=True)
