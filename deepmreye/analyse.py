...
def visualise_input_data(X,
    ...
    fig.add_trace(go.Scatter(x=np.arange(0, len(vis_x)),
                             y=vis_x,
                             mode='lines',
                  ...
                  row=1,
                  col=3)
    fig.add_trace(go.Scatter(x=np.arange(0, len(vis_y)),
                             y=vis_y,
                             mode='lines',
                             line_color='rgb(0,0,0)',
                             opacity=0.5,
    ...
    for i in range(0, X_vis.shape[0]):
        ...
        fig.add_heatmap(
            ...
            col=1,
            name="TR: {}".format(
                i))  # colorbar=dict(x=0.45, y=0.5, thickness=10, len=0.5))
    fig.data[num_functionals * 3 + 2].visible = True  # Some arithmetic;
    fig.data[num_functionals * 3 + 2 +
             1].visible = True  # connects to the active value below;
    fig.data[num_functionals * 3 + 2 +
             2].visible = True  # should be abstracted as a variable;

    # Add slider for changing TR
    steps, stepcount = [], 0
    for i in range(2, len(fig.data) - 2, 3):
        step = dict(method="update",
                    label="TR: {}".format(stepcount),
                    args=[{
    ...
    fig.update_xaxes(range=[-2, 150 + 2],
                     row=1,
                     col=3,
                     title=dict(text='Functional Volume (TR)',
                                standoff=16,
                                font=dict(size=20)))
...
def visualise_predictions_click(evaluation,
                                scores,
                                color="rgb(0, 150, 175)",
                                bg_color="rgb(247,247,247)"):
    # Prepare data for plotting
    ...
    to_plot = np.concatenate((all_scores[..., 2], all_scores[..., 5]),
                             axis=0)  # Pearson Mean & R2 Mean
    x = ['Pearson'] * all_scores[..., 2].shape[0] + ['R^2-Score'] * all_scores[
        ..., 5].shape[0]
    ...
    fig.add_trace(go.Box(y=to_plot[:, 0],
                         marker_size=12,
                         x=x,
                         boxpoints='all',
                         pointpos=0,
                  ...
                  row=1,
                  col=1)
    fig.add_trace(go.Box(y=to_plot[:, 1],
                         marker_size=12,
                         x=x,
                         boxpoints='all',
                         pointpos=0,
                  ...
                  row=1,
                  col=1)
    fig.add_trace(go.Box(y=to_plot[:, 2],
                         marker_size=12,
                         x=x,
                         boxpoints='all',
                         pointpos=0,
                  ...
                  row=1,
                  col=1)
    fig.add_trace(go.Box(y=to_plot[:, 3],
                         marker_size=12,
                         x=x,
                         boxpoints='all',
                         pointpos=0,
    ...
    fig.add_trace(go.Scatter(x=np.arange(0, len(this_real[:, 0])),
                             y=this_real[:, 0],
                             mode='lines',
                             line_color='rgb(0,0,0)',
                             opacity=0.5,
                  ...
                  row=1,
                  col=3)
    fig.add_trace(go.Scatter(x=np.arange(0, len(this_pred[:, 0])),
                             y=this_pred[:, 0],
                             mode='lines',
                             line_color=color,
                             opacity=0.85,
                  ...
                  row=1,
                  col=3)
    fig.add_trace(go.Scatter(x=np.arange(0, len(this_real[:, 1])),
                             y=this_real[:, 1],
                             mode='lines',
                             line_color='rgb(0,0,0)',
                             opacity=0.5,
                  ...
                  row=2,
                  col=3)
    fig.add_trace(go.Scatter(x=np.arange(0, len(this_pred[:, 1])),
                             y=this_pred[:, 1],
                             mode='lines',
                             line_color=color,
                             opacity=0.85,
    ...
    def update_point(trace, points, selector):
        if points.point_inds:
            ...
            this_real = np.median(evaluation[this_participant]['real_y'],
                                  axis=1)
            this_pred = np.median(evaluation[this_participant]['pred_y'],
                                  axis=1)
            ...
            fig.update_xaxes(range=[-2, 150 + 2],
                             row=1,
                             col=3,
                             title=dict(text='Input Volume (TR)',
                                        standoff=16,
                                        font=dict(size=20)))
            fig.update_xaxes(range=[-2, 150 + 2], row=2, col=3)
...
def visualise_predictions_slider(evaluation,
                                 scores,
                                 color="rgb(0, 150, 175)",
                                 bg_color="rgb(247,247,247)",
    ...
    for key, item in evaluation.items():
        with warnings.catch_warnings():
            ...
            if subTR:
                this_real = np.reshape(
                    item['real_y'],
                    (item['real_y'].shape[0] * item['real_y'].shape[1], -1))
    ...
    for i in range(4, len(fig.data), 4):
        ...
        this_sub = os.path.splitext(os.path.basename(
            participants[stepcount]))[0]
        step = dict(method="update",
                    label="{}".format(this_sub),
                    args=[{
                        "visible": 4 * [True] + [False] * (len(fig.data) - 4)
                    }])
        for j in range(0, 4):
    ...
    fig.update_xaxes(range=[x_start, x_range + x_start],
                     row=1,
                     col=3,
                     title=dict(text='Functional Volume (TR)',
                                standoff=16,
