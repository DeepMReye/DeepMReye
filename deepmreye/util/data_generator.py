...
def create_generators(full_training_list,
                      full_testing_list,
                      batch_size=8,
    ...
    testing_generator = data_generator(full_testing_list,
                                       batch_size,
                                       training=False,
                                       inner_timesteps=inner_timesteps,
                                       mixed_batches=mixed_batches,
    ...
    single_testing_generators, single_testing_names = get_single_data_generators(
        full_testing_list,
        batch_size,
        string_cut=7,
        training=False,
        ...
        withinsubject_split=withinsubject_split,
        inner_timesteps=inner_timesteps)
    single_training_generators, single_training_names = get_single_data_generators(
        full_training_list,
        batch_size,
        string_cut=7,
        training=True,
    ...
    training_subjects_string = [
        os.path.splitext(os.path.basename(p))[0] for p in full_training_list
    ]
    test_subjects_string = [
        os.path.splitext(os.path.basename(p))[0] for p in full_testing_list
    ]

    print(util.color.BOLD + 'Training set ({}) contains {} subjects: '.format(
        os.path.dirname(full_training_list[0]), len(full_training_list)) +
          util.color.END)
    pprint(training_subjects_string, compact=True)
    print(util.color.BOLD + 'Test set ({}) contains {} subjects: '.format(
        os.path.dirname(full_testing_list[0]), len(full_testing_list)) +
          util.color.END)
    pprint(test_subjects_string, compact=True)
    # Should be a dict.
    return (training_generator, testing_generator, single_testing_generators,
            single_testing_names, single_training_generators,
...
def create_holdout_generators(datasets, train_split=0.6, **args):
    ...
    (training_generator, testing_generator, single_testing_generators,
     single_testing_names, single_training_generators,
     single_training_names) = create_generators(full_training_list,
                                                full_testing_list, **args)

    return (training_generator, testing_generator, single_testing_generators,
            single_testing_names, single_training_generators,
            single_training_names, full_testing_list, full_training_list)
...
def create_cv_generators(dataset, num_cvs=5, **args):
    ...
    for idx, cvs in enumerate(cv_split):
        ...
        full_training_list = np.concatenate(
            [x for i, x in enumerate(cv_split) if i != idx]).tolist()

        (training_generator, testing_generator, single_testing_generators,
         single_testing_names, single_all_generators,
         single_all_names) = create_generators(full_training_list,
                                               full_testing_list, **args)
        cv_return.append(
            (training_generator, testing_generator, single_testing_generators,
             single_testing_names, single_all_generators, single_all_names,
...
def create_leaveoneout_generators(datasets, training_subset=None, **args):
    ...
    for idx, dataset in enumerate(datasets):
        ...
        full_training_list = [
            tds + p for tds in training_datasets for p in os.listdir(tds)
        ]

        if training_subset is not None:
            size_before = len(full_training_list)
            full_training_list = [
                tds for tds in full_training_list
                if os.path.basename(os.path.dirname(tds)) + '/' +
                os.path.basename(tds) in training_subset
            ]
            print('Using subset ({} / {}) for training {}'.format(
                len(full_training_list), size_before, dataset))
...
def data_generator(file_list,
    ...
    if withinsubject_split is None:
        file_list = [
            file_list[x] for x in range(0, len(all_nonan_indices))
            if all_nonan_indices[x].size > 0
        ]
        all_nonan_indices = [
            all_nonan_indices[x] for x in range(0, len(all_nonan_indices))
            if all_nonan_indices[x].size > 0
        ]
    else:
        ...
        for x in range(0, len(all_nonan_indices)):
            try:
                get_subject_data(file_list[x],
                                 batch_size=1,
                                 start_tr=start_tr,
                                 end_tr=end_tr,
                                 nonan_indices=all_nonan_indices[x])
    ...
    while True:
        if mixed_batches:
            fn_subjects, nonan_indices = zip(*random.choices(
                list(zip(file_list, all_nonan_indices)), k=batch_size))
            X, y = [], []
            for subj, nans in zip(fn_subjects, nonan_indices):
                tmp_X, tmp_y = get_subject_data(subj,
                                                batch_size=1,
                                                start_tr=start_tr,
                                                end_tr=end_tr,
                                                nonan_indices=nans)
            ...
            X, y = get_subject_data(fn_subject,
        ...
        if inner_timesteps is not None:
            y = [
                y_tmp[np.linspace(
                    0, y[0].shape[0] - 1, inner_timesteps, dtype=int), :]
                for y_tmp in y
            ]
        X, y = np.array(X)[..., np.newaxis], np.array(y)
        # Augmentation
        if training and any(augment_list):
            X = util.augment_input(X,
                                   rotation=augment_list[0],
                                   shift=augment_list[1],
                                   zoom=augment_list[2])

        yield [[X, y], []]


def get_subject_data(fn_subject,
                     batch_size=None,
                     sample_index=None,
                     start_tr=None,
                     end_tr=None,
    ...
    if batch_size is not None:
        ...
        nonan_indices = nonan_indices[(nonan_indices >= start_tr)
                                      & (nonan_indices <= end_tr)]
...
def get_all_subject_data(fn_subject):
    ...
    subject_data_X, subject_data_y = np.array(subject_data_X), np.array(
        subject_data_y)
