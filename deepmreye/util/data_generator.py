...
def create_generators(
    ...
    testing_generator = data_generator(
        full_testing_list,
        batch_size,
        training=False,
        inner_timesteps=inner_timesteps,
    ...
    print(util.color.BOLD + "Training set ({}) contains {} subjects: ".format(
        os.path.dirname(full_training_list[0]),
        len(full_training_list)) + util.color.END)
    pprint(training_subjects_string, compact=True)
    print(util.color.BOLD + "Test set ({}) contains {} subjects: ".format(
        os.path.dirname(full_testing_list[0]),
        len(full_testing_list)) + util.color.END)
...
def create_holdout_generators(datasets, train_split=0.6, **args):
    ...
    (
        ...
        single_training_generators,
        single_training_names,
    ) = create_generators(full_training_list, full_testing_list, **args)
    ...
    for idx, dataset in enumerate(datasets):
        ...
        if training_subset is not None:
            ...
            full_training_list = [
                ...
                if os.path.basename(os.path.dirname(tds)) + "/" + os.path.basename(tds) in training_subset
            ]
