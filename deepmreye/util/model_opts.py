...
def get_opts():
    ...
    opts[
        "multiplier"] = 2  # How much does the number of filters increase in each layer
    ...
    opts[
        "gaussian_noise"] = 0  # How much gaussian noise is added (unit = standard deviation)
    ...
    opts[
        "inner_timesteps"] = 10  # Default number of subTR samples which are being reconstructed
    ...
    opts[
        "train_test_split"] = 0.6  # Default proportion of train (60%)-test(40%) split
    opts["batch_size"] = 8  # Batch size used for training the model
    opts[
        "mixed_batches"] = True  # If true, each batch contains samples across participants
