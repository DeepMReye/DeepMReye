...
def test_masks(path_to_masks):
    (eyemask_small, eyemask_big, dme_template, mask, x_edges, y_edges,
     z_edges) = preprocess.get_masks(path_to_masks)
...
def test_example_participant(path_to_masks, path_to_testdata):
    ...
    (masked_eye, transformation_statistics) = preprocess.run_participant(
        path_to_participant, dme_template, eyemask_big, eyemask_small, x_edges,
        y_edges, z_edges)
    ...
    this_label = preprocess.load_label(path_to_testdata,
                                       label_type='calibration_run')
    this_label = this_label[0:this_mask.shape[3], ...]  # Adjust for testing
    this_id = ['test_participant'
               ] * this_label.shape[0], [0] * this_label.shape[0]

    preprocess.save_data('test_participant', [this_mask], [this_label],
                         [this_id],
                         path_to_testdata + 'processed/',
                         center_labels=False)
...
def test_model_training(path_to_testdata):
    ...
    generators = data_generator.create_leaveoneout_generators(
        ...
        augment_list=((opts['rotation_x'], opts['rotation_y'],
                       opts['rotation_z']), opts['shift'], opts['zoom']),
        mixed_batches=True)
    ...
    # Evaluate model
    (evaluation, scores) = train.evaluate_model(dataset='example_data',
